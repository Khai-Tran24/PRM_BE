...
namespace BE_SaleHunter.Application.Services
{
    public interface IProductService
    {
        ...
        Task<BaseResponseDto<IEnumerable<ProductDto>>> SearchProductsAsync(
            ProductSearchRequestDto searchRequest, PagingRequestDto pagingRequest);
        ...
        : IProductService
    {
        ...
        public async Task<BaseResponseDto<IEnumerable<ProductDto>>> SearchProductsAsync(
            ProductSearchRequestDto searchRequest, PagingRequestDto pagingRequest)
        {
            try
            {
                var products =
                    await unitOfWork.ProductRepository.SearchProductsAsync(
                        pagingRequest.Page,
                        pagingRequest.Size,
                        searchRequest.Query ?? string.Empty,
            ...
            {
                logger.LogError(ex, "Error searching products with query: {Query}", searchRequest.Query);
        ...
        public async Task<BaseResponseDto<IEnumerable<ProductDto>>> GetRecommendedProductsAsync(long userId)
        {
            try
            {
                ...
                var favoriteProducts = await unitOfWork.ProductRepository.GetAllProductsWithDetailAsync();
                var recommendedProducts = favoriteProducts.Where(p => favoriteProductIds.Contains(p.Id)).Take(20).ToList();
